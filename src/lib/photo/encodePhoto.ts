...
/** 결과가 상한을 넘으면 이 순서로 품질을 낮춰 다시 굽는다. */
const QUALITY_LADDER = [JPEG_QUALITY, 0.72, 0.6] as const
 ...
 * HEIC/HEIF 인지 본다.
 *
 * 정확 일치만으로는 image/heic-sequence 같은 변형과, 일부 안드로이드
 * 콘텐츠 제공자가 주는 빈 MIME 을 놓친다. 그러면 '디코드 실패' 로만 보여서
 * 카메라 설정 탓이라는 걸 알 수 없다. 파일 이름도 같이 본다.
 */
...
export async function encodePhoto(file: Blob): Promise<EncodedPhoto> {
  ...
  // MIME 이 비어 있는 경우가 있어 명시적으로 아닌 것만 거른다.
  ...
  try {
    ...
    //
    // 상한을 넘으면 바로 포기하지 않고 품질을 낮춰 다시 굽는다. 사용자는 그냥
    // 문장을 저장하고 싶을 뿐이므로, 조금 흐려지더라도 되는 편이 낫다.
